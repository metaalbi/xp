(function () {
    ...
    const startMenuPrograms = document.getElementById('startMenuPrograms');
    const startMenuShortcuts = document.getElementById('startMenuShortcuts');
    ...
    const startMenuLogOff = document.getElementById('startMenuLogOff');
...
${lines.map((line) => `                ${line}`).join('\n')}
    ...
    function buildStartMenu() {
        ...
        startMenuShortcuts.innerHTML = '';

        (data.desktopIcons || []).forEach((iconData) => {
            const button = document.createElement('button');
            button.type = 'button';
            button.className = 'start-menu-program';
            button.dataset.window = iconData.id;
            button.setAttribute('role', 'menuitem');
            button.innerHTML = `
                <span class="start-menu-icon ${iconData.icon}"></span>
                <span class="start-menu-label">${iconData.label}</span>
            `;
            ...
            startMenuPrograms.appendChild(button);
        });

        const contact = data.contact || {};
        const defaultShortcuts = [
            { label: 'My Documents', description: 'Experience & projects', windowId: 'window-experience', icon: 'icon-folder' },
            { label: 'About Me', description: 'Summary & background', windowId: 'window-about', icon: 'icon-folder' },
            { label: 'Contact', description: 'Email & availability', windowId: 'window-contact', icon: 'icon-notepad' }
        ];

        if (contact.resumeUrl) {
            defaultShortcuts.splice(1, 0, {
                label: 'Resume (PDF)',
                description: 'Download the latest CV',
                icon: 'icon-notepad',
                url: contact.resumeUrl
            });
        }

        const shortcuts = Array.isArray(data.startMenuShortcuts) && data.startMenuShortcuts.length
            ? data.startMenuShortcuts
            : defaultShortcuts;

        shortcuts.forEach((shortcut) => {
            const item = document.createElement('button');
            item.type = 'button';
            item.className = 'start-menu-shortcut';
            item.setAttribute('role', 'menuitem');
            item.innerHTML = `
                <span class="start-menu-icon ${shortcut.icon || 'icon-folder'}"></span>
                <span class="start-menu-label">
            ...
            if (shortcut.windowId) {
                ...
                item.addEventListener('click', () => {
                    const win = window.open(shortcut.url, shortcut.target || '_blank', 'noopener,noreferrer');
                    if (win) {
                        win.opener = null;
                    }
                    closeStartMenu();
                });
            } else if (shortcut.mailto || contact.email) {
                const email = shortcut.mailto || contact.email;
                item.addEventListener('click', () => {
            ...
            startMenuShortcuts.appendChild(item);
        });
    }
    ...
    function logOffUser() {
        windowsRegistry.forEach((_, id) => closeWindow(id));
        closeStartMenu();
        desktop.focus?.();
    }
    ...
    if (startMenuLogOff) {
        startMenuLogOff.addEventListener('click', logOffUser);
    }

    if (startMenuShutDown) {
        startMenuShutDown.addEventListener('click', shutDownSystem);
    }
